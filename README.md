# PSOC&trade; Control C1 MCU: SPI master

This code example demonstrates the transfer of three bytes of data using the SPI master in PSOC&trade; Control C1 MCU. The successful completion of transfer is indicated by the toggling of an LED on the board.

[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce242890-spi-master)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
| --- | --- | --- | --- | --- | --- |
| KIT_PSC1M3_EVK | 1[0] | 2[0] | 0[8] | 0[9] | GND |
...
The following example clones the "[SPI master](https://github.com/Infineon/mtb-example-ce242890-spi-master)" application with the desired name "SPI master" configured for the *KIT_PSC1M3_EVK* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSC1M3_EVK --app-id mtb-example-ce242890-spi-master --user-app-name SPIMaster --target-dir "C:/mtb_projects"
   ```
...
4. Check whether the 'User LED1' toggles, which confirm the completion of the transfer of three bytes
...
In this code example, the SPI block of the PSOC&trade; Control C1 MCU is configured as the SPI master to send three bytes of data to the SPI slave. After the initialization of the SPI peripheral, the input stage for synchronous serial communication (SSC/SPI) and the SPI channel are set for communication.
...
| :------- | :------------    | :------------ |
| SPI (PDL)  | SPI0 | SPI master transfers 3 bytes to the SPI slave |
...
Document title: *CE242890* – *PSOC&trade; Control C1 MCU: SPI master*
